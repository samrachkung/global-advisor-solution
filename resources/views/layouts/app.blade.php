...
<head>
  ...
  @foreach($locales as $loc)
    <link rel="alternate" hreflang="{{ $loc }}" href="{{ url($loc === app()->getLocale() ? request()->path() : $loc.'/'.request()->path()) }}">
  @endforeach
  ...
  <link rel="preload" as="style" href="https://fonts.googleapis.com/css2?family=Kantumruy+Pro:wght@400;600;700&display=swap">
  ...
  <script>
    ...
    window.addEventListener('load', function () {
      if ('requestIdleCallback' in window) {
        requestIdleCallback(() => { try { AOS.init({ duration: 700, once: true }); } catch(e) {} });
      } else {
        setTimeout(() => { try { AOS.init({ duration: 700, once: true }); } catch(e) {} }, 0);
      }
    });
  </script>
  <noscript>
